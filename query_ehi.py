...
from config import W_SQS, W_FDS, W_FTP, W_SPS, W_DES
...
def _ehi_components(sqs, fds, ftp, sps, des):
    return [
        ("SQS", W_SQS, sqs),
        ("FDS", W_FDS, fds),
        ("FTP", W_FTP, ftp),
...
def get_player_game(player_name: str, date: str) -> dict | None:
    ...
    r      = rows[0]
    name, team, pos, date_, min_, pts, sqs, fds, ftp, sps, des, ehi, home, away, season = r
    matchup    = f"{home} vs {away}"
    components = _ehi_components(sqs, fds, ftp, sps, des)
    BAR_W      = 22
    W          = 80
    ...
    return {
        ...
        "min": min_, "pts": pts,
        "SQS": sqs, "FDS": fds, "FTP": ftp, "SPS": sps, "DES": des, "EHI": ehi,
    }
...
def get_player_season(player_name: str, season: str) -> dict | None:
    ...
    with get_connection() as conn:
        ...
        rows = conn.execute(
            ...
            SELECT pg.player_name,
                   ...
                   AVG(pg.SPS), AVG(pg.DES), AVG(pg.EHI),
    ...
    r = rows[0]
    name, avg_sqs, avg_fds, avg_ftp, avg_sps, avg_des, avg_ehi, gp, avg_pts, min_ehi, max_ehi = r
    components = _ehi_components(avg_sqs, avg_fds, avg_ftp, avg_sps, avg_des)
    BAR_W      = 22
    W          = 80
...
def get_game_leaderboard(date: str, team1: str, team2: str) -> list[dict] | None:
    ...
    print(
        ...
        f"{'SQS':>7}{'FDS':>7}{'FTP':>7}{'SPS':>7}{'DES':>7}  │{'EHI':>7}"
    )
    print("  " + "─" * (W - 2))
    ...
    for rank, r in enumerate(rows, 1):
        name, team, pos, min_, pts, sqs, fds, ftp, sps, des, ehi = r
        print(
            f"  {rank:>3}  {name:<24}{pos:>5}{team:>4}{min_:>6.1f}{pts:>5}"
            f"{sqs:>7.1f}{fds:>7.1f}{ftp:>7.1f}{sps:>7.1f}{des:>7.1f}  │{ehi:>7.2f}"
        )
...
def _season_ranking(season: str, n: int, ascending: bool) -> list[dict] | None:
    ...
    with get_connection() as conn:
        rows = conn.execute(
            ...
            LIMIT ?
            """,
            (season, season, MIN_GAMES, n),
        ).fetchall()

    if not rows:
        print(f"  No data for {season} with >= {MIN_GAMES} games played.")
        return None

    W = 102
    print("\n" + "=" * W)
    print(f"EHI Season {label} {n} — {season}  (min {MIN_GAMES} games)")
    print(f"  EHI = {W_SQS}×SQS + {W_FDS}×FDS + {W_FTP}×FTP + {W_SPS}×SPS + {W_DES}×DES")
    print("=" * W)
    print(
        f"  {'Rk':>3}  {'Player':<24}{'Tm':>4}{'GP':>4}{'aPTS':>6}"
        f"{'aSQS':>7}{'aFDS':>7}{'aFTP':>7}{'aSPS':>7}{'aDES':>7}  │{'aEHI':>7}"
    )
    print("  " + "─" * (W - 2))

    result: list[dict] = []
    for rank, r in enumerate(rows, 1):
        name, team, avg_sqs, avg_fds, avg_ftp, avg_sps, avg_des, avg_ehi, gp, avg_pts = r
        team = team or "—"
        print(
            f"  {rank:>3}  {name:<24}{team:>4}{gp:>4}{avg_pts:>6.1f}"
            f"{avg_sqs:>7.1f}{avg_fds:>7.1f}{avg_ftp:>7.1f}{avg_sps:>7.1f}{avg_des:>7.1f}"
            f"  │{avg_ehi:>7.2f}"
        )
        result.append({
            "player":   name,
...
def get_season_best(season: str, n: int = 10) -> list[dict] | None:
    """Top N players by avg EHI for the season, minimum MIN_GAMES games played."""
    return _season_ranking(season, n, ascending=False)


def get_season_worst(season: str, n: int = 10) -> list[dict] | None:
    """Bottom N players by avg EHI for the season, minimum MIN_GAMES games played."""
    return _season_ranking(season, n, ascending=True)
